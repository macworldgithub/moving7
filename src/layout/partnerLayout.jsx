...
import {
  getIsPartnerWappVerified,
  requestPartnerEmailOTP,
  requestPartnerWAppOTP,
...
const PartnerLayout = ({ user, setUser }) => {
  ...
  const isPartnerVerified =
    fetchPartnerWappStatus?.data?.data?.wappVerified ?? true;
  const isPartnerEmailVerified =
    fetchPartnerWappStatus?.data?.data?.emailVerified ?? true;
  ...
  useEffect(() => {
    ...
    if (
      !user?.proof &&
      window.location.pathname !== "/partner/documentsVerification"
    )
  ...
  useEffect(() => {
    if (isPartnerVerified === false) {
      setShowVerificationModal(true);
      console.log(user);
    } else if (isPartnerEmailVerified === false && isPartnerVerified) {
      console.log("idhar");
          ...
          <div className="bg-white rounded-lg h-48 w-96">
            ...
            <div className="flex justify-center mt-4">
              <VerifyOTP
                ...
                showModal={setShowVerificationModal}
              />
            </div>
          </div>
        </MyModal>
        ...
        <MyModal>
          ...
          <div className="bg-white rounded-lg h-48 w-96">
            ...
            {isButtonDisabled ? (
              <>
                <p className="text-center text-gray-500">
                  A verification message is sent on your WhatsApp! Please
                  verify.
                </p>
                <div className="flex justify-center mt-4">
                  <VerifyOTP
              ...
              <div className="h-[50%] flex flex-col justify-between">
                ...
                <div className="w-full bg- flex mt-2 justify-center">
                  <button
                    ...
                    className={` rounded text-black px-5 text-xs py-1 ${isButtonDisabled ? "bg-slate-400" : "bg-primary"} `}
                    onClick={() => getOtpMutation.mutate(user.telephone)}
                  >
                    Send OTP
                  </button>
                </div>
