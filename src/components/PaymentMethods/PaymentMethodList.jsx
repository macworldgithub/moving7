...
import { detachPaymentMethod, fetchPaymentMethods, getStripeCustomer, setDefaultPM } from '../../apiFunctions/partner';
...
const PaymentMethodList = () => {
    ...
    const defaultPaymentMethod = fetchStripeCustomer?.data?.data?.invoice_settings?.default_payment_method ?? ""
                        ...
                        return (
                            <div className="my-2 rounded-lg justify-between items-center border py-2 px-4 flex">
                                ...
                                <div className="flex items-center gap-3">
                                    <div onClick={() => handleDefaultpaymentMethod(pm?.id)} className="border cursor-pointer rounded-full bg-primary text-xs py-1 px-3  border-primary">
                                        {
                                            defaultPaymentMethod === pm?.id ? "default" : "set as default"
                                        }
                                    </div>
                                    <FaTrash onClick={() => handleDelete(pm?.id)} className="cursor-pointer text-gray-400" />
                                </div>
                            </div>
                        )
                    })
