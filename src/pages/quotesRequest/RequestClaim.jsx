...
const Claim = ({ status, quotationData }) => {
    ...
    return (
        <>
            <AntdModal title={"Submit your claim"} handleSubmit={() => {
            ...
            }} isModalOpen={showClaimModal} setIsModalOpen={setShowClaimModal} >
                {
                    claim?.claim ? (
                        <div className="flex flex-col w-full">
                            ...
                            <div className="flex justify-between">
                                ...
                                <p>
                                    {new Date(claim?.claim?.claimedAt).toLocaleString()}
                                </p>
                            </div>
                            <p className="text-gray-400">
                                You won't be charged for this lead if status is Submitted or Accepted.
                            </p>
                        </div>
                    )
                        : (
                            <div>
                                <p className='text-base mb-1'>
                                    Kindly specify the reason.
                                </p>
                                <textarea value={claimReason} onChange={(e) => setClaimReason(e.target.value)} className='w-full border h-40 py-2 px-2' />
                            </div>
                        )
                }
            </AntdModal>
