...
const DocumentVerification = () => {
  ...
  const submit = async () => {
    ...
    let keys = ["license", "VATcert", "emiratesId", "insuranceCert"];
    let labels = [
      "License",
      "VAT Certificate",
      "Emirates Id",
  ...
  console.log(files, "Selectedddddd", isValid());
  ...
  return (
    <div className="items-center flex justify-center">
      <div className="mt-20 bg-primary p-10 md:w-3/5 sm:w-4/5 ssm:w-4/5 rounded-3xl">
        ...
        <p className="w-4/5 text-black text-para mt-3">
          Ensuring Legitimacy and Quality in Service Platforms
        </p>
        <ul className="text-para">
          <li>1. Introduction</li>
          <li>2. Importance of Genuine Professionals in Service Platforms</li>
          <li>3. Verification Processes for Companies</li>
          <li>4. Benefits of Authenticity and Quality for Consumers</li>
          <li>5. Sky Rocket your Revenue</li>
        </ul>

        <div className="flex flex-wrap">
          <div className="md:w-48 sm:w-48 lg:w-1/2">
            <div className="flex items-start mt-3">
              <Avatar size={30} className="bg-blue-300 me-3 mt-">
                {selectedItem + 1}
              </Avatar>
              <p>Upload a {files[selectedItem]?.name}</p>
            </div>
            <div className="h-max min-h-80 flex justify-center rounded-3xl mt-4 mb-1 bg-white">
              <div className="flex flex-col items-center my-28 justify-center">
                {files[selectedItem]?.file ? (
                  ...
                  <>
                    <p className="text-gray-500 my-1">Drag files to upload</p>
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (size > 20) {
                            toast.error(
                              "File greater than 20mb cannot be uploaded! ",
                            );
                            return;
                          }
          ...
          <div className="md:w-48 sm:w-48 lg:w-1/2 px-10">
            <div className="mt-16">
              {files.map((elem, idx) => {
                return (
                  ...
                  >
                    <h1 className="font-medium text-lg p-0 m-0">{elem.name}</h1>
                    <DatePicker
                      onChange={(_, dateStr) => {
                        setFiles((prev) => {
                          let temp = prev;
                    ...
                    <Progress
                      percent={elem.status ? 100 : 0}
                      className="p-0 m-0"
                    />
                    <p className="text-gray-500 text-sm p-0 m-0">
                      {elem?.file
                        ? elem.file?.name?.slice(0, 40) + "..."
                        : "Not selected"}{" "}
                      <br />
            ...
            <button
              onClick={submit}
              className={` text-black px-16 rounded-full py-1  ${isValid() ? "bg-gray-400 text-white" : "bg-blue-300 text-black"}`}
            >
